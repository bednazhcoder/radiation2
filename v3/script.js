// ════════════════════════════════════════════════════════
  ...
  function renderHeatmap(){
    // Leaflet.heat erwartet [[lat, lon, intensity], ...]
    // Intensität = µSv/h-Wert (bereits in Rohwerten, normalisiert intern)
    const pts = measurements
      .filter(m=>!isNaN(Number(m.lat))&&!isNaN(Number(m.lon)))
      .map(m=>[Number(m.lat), Number(m.lon), m.value]);
    ...
    if(typeof L.heatLayer === "undefined"){
      document.getElementById("mapStatus").textContent = "⚠️ Heatmap-Plugin konnte nicht geladen werden.";
      return;
    }
    ...
    L.heatLayer(pts, {
      ...
      blur: 35,
      maxZoom: 10,
      max: Math.max(...pts.map(p=>p[2])),
      gradient: {
        0.0:  "#00008b",   // dunkelblau  – kälteste Zone (sehr niedrig)
        0.25: "#0080ff",   // blau
        0.45: "#00e0a0",   // türkis
        0.65: "#ffee00",   // gelb
        0.80: "#ff8800",   // orange
        1.0:  "#cc0000"    // rot         – heißeste Zone (höchste Strahlung)
      }
    }).addTo(map);
    ...
    const heatGrad = "linear-gradient(90deg, #00008b 0%, #0080ff 25%, #00e0a0 45%, #ffee00 65%, #ff8800 80%, #cc0000 100%)";
    const vals = pts.map(p=>p[2]);
    const minV=Math.min(...vals), maxV=Math.max(...vals);
    legendEl.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;width:100%;">
        <span style="font-weight:700;color:var(--text-dim);white-space:nowrap;font-size:12px;">Intensität:</span>
        <div style="display:flex;align-items:center;gap:8px;flex:1;min-width:220px;max-width:420px;">
          <span style="font-size:12px;font-weight:700;white-space:nowrap;">${formatVal(minV)}</span>
